...
describe("Enroll Feature", () => {
  const sut_service = container.resolve(EnrollService);
  ...
  it("should enroll a user to a course", async () => {
    ...
    const course = await prisma.course.create({
      data: { id: request.course_id, name: faker.commerce.productName() },
    });
  ...
  it("should not enroll a user to more than 3 courses", async () => {
    ...
    await prisma.course.create({
      data: { id: request_1.course_id, name: faker.commerce.productName() },
    });
    await prisma.course.create({
      data: { id: request_2.course_id, name: faker.commerce.productName() },
    });
    await prisma.course.create({
      data: { id: request_3.course_id, name: faker.commerce.productName() },
    });
    await prisma.course.create({
      data: { id: request_4.course_id, name: faker.commerce.productName() },
    });
